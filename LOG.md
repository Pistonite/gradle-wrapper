...
**Pre-9.0 fallback is untested.** `find_launcher_jar()` prefers `gradle-gradle-cli-main-*.jar` and falls back to `gradle-launcher-*.jar`, but every test ran against 9.6.1, which ships both — so the fallback branch has never actually been taken. PLAN.md's verification step 9 (test an 8.x `distributionUrl`) is still outstanding.
